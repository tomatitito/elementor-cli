...
PAGE1_ID=$(wp post create \
  --post_type=page \
  --post_title="Simple Page" \
  --post_status=publish \
  --post_name="simple-page" \
  --porcelain \
  --path=/var/www/html \
  --allow-root)

echo "Created page $PAGE1_ID: Simple Page"
...
PAGE2_ID=$(wp post create \
  ...
  --post_status=publish \
  --post_name="complex-page" \
  --porcelain \
  --path=/var/www/html \
  --allow-root)

echo "Created page $PAGE2_ID: Complex Page"
...
echo "Created page $PAGE3_ID: Draft Page"
...
# Delete default WordPress pages (Sample Page and Privacy Policy)
echo "Cleaning up default WordPress pages..."
wp post delete $(wp post list --post_type=page --field=ID --title="Sample Page" --path=/var/www/html --allow-root 2>/dev/null || echo "") --force --path=/var/www/html --allow-root 2>/dev/null || true
wp post delete $(wp post list --post_type=page --field=ID --title="Privacy Policy" --path=/var/www/html --allow-root 2>/dev/null || echo "") --force --path=/var/www/html --allow-root 2>/dev/null || true

# Create a menu with all pages
echo "Creating menu with Elementor pages..."
MENU_ID=$(wp menu create "Main Menu" --porcelain --path=/var/www/html --allow-root)
wp menu item add-post "Main Menu" $PAGE1_ID --path=/var/www/html --allow-root
wp menu item add-post "Main Menu" $PAGE2_ID --path=/var/www/html --allow-root
wp menu item add-post "Main Menu" $PAGE3_ID --path=/var/www/html --allow-root
...
echo "  - Page $PAGE1_ID: Simple Page (elementor_canvas)"
echo "  - Page $PAGE2_ID: Complex Page (elementor_header_footer)"
echo "  - Page $PAGE3_ID: Draft Page (published)"
echo ""
